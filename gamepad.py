...
class Gamepad:
    def __init__(self, i2c_bus=0, scl_pin=1, sda_pin=0, addr=0x50):
    ...
    def get_joystick_state(self):
        ...
        newly_pressed_buttons = [name for name, btn in zip(self.BTN_Value, buttons) if btn]

        # Store the previous button state to detect newly pressed buttons
        if not hasattr(self, 'previous_buttons'):
            self.previous_buttons = buttons

        newly_pressed_buttons = [name for name, btn, prev_btn in zip(self.BTN_Value, buttons, self.previous_buttons) if btn and not prev_btn]

        # Update the current_button variable
        if newly_pressed_buttons:
            self.current_button = newly_pressed_buttons[0]
        else:
        ...
        return x, y, newly_pressed_buttons
